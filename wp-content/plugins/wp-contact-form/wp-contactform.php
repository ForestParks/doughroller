	...
	function wpcf_callback($content) {
		...
		if (strpos($content,'%%wpcontactform%%')!== false) {
			if (wpcf_check_input()) {
				...
				$form.= '<small>Highlighted fields are required.</small>';
				$form.= '<input type="hidden" name="wpcf_stage" value="process" /></td></tr>';
				$form.= '<tr><td style="text-align:right">'. __('Your Name: ','wpcf').'</td>';
				$form.= '<td style="text-align:left">'.$wpcf_strings['name'].'</td></tr>';
				$form.= '<tr><td style="text-align:right">'. __('Your Email:','wpcf').'</td>';
				...
				$form.= '<tr><td style="text-align:right">'. __('Your Website:','wpcf').'</td>';
				...
				if(strpos(get_option('wpcf_subject'),"|")>0) {
					...
					if (get_option('wpcf_showsubject')=="TRUE") {
						$form.= '<tr><td style="text-align:right">'. __('Your Subject:','wpcf').'</td>';
						$form.= '<td style="text-align:left">'.$wpcf_strings['usersubject'].'</td></tr>';
					}
				}
				$form.= '<tr><td style="text-align:right">'. __('Your Message: ','wpcf').'</td>';
