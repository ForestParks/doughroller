...
;(function($){
	$(document).ready(function(){
		...
		init_tabs();
		$('body').on('click','#popup_domination_active a',function(){
		$('#popup_domination_active .waiting').show();
	...
	function activate(resp){
		...
		if(resp.error){
			...
			$('#popup_domination_active').html('<span class="wording"><span class="'+class1+'">'+txt+'</span> PopUp Domination is '+txt3+' </span><div class="popup_domination_activate_button"><div class="border">'+txt2+'<a href="#activation" class="'+class2+'"> '+txt5+' </a></div></div><img class="waiting" style="display:none;" src="../wp-includes/images/wpspin.gif" alt="" />');
		} else {
			alert(resp);
		}
