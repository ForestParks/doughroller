...
pty = {
	...
	checkPowerFilters: function(){
		...
		var checkDepth = function(filters, depth) {
			depth += 1;
			$j.each(filters, function(i, filter){
				if (filter.and !== undefined && filter.and.length > 0) {
					var retDepth = checkDepth(filter.and, depth);
					if (retDepth > depth) {
						depth = retDepth;
					}
				}
		...
		if (checkDepth(pty.theme.filters, 1) > 6) {
			$j('#pty_filters').css('overflow-x', 'auto');
			$j('.pty_aFilter').css('width', '400px').css('background', 'rgba(246, 248, 225, 1)');
			$j('.pty_aFilterType').css('border-radius', '0 6px 0 0 ');
		}
		else {
			$j('.pty_aFilter').css('width', '').css('background', '');
			$j('#pty_filters').css('overflow-x', '');
			$j('.pty_aFilterType').css('border-radius', '');
		}
	},
	...
	processNewsletterHtml: function(go) {
		...
		if (pty.isTribulantShortcode(val)) {
			...
			//                 
				...
				$j('div', form).each(function(){
					if ($j(this).css('position') == 'absolute') {
						var honey = $j('input', $j(this)).attr('name')
						$j('input', $j(this)).remove();
						$j(form).append($j('<input/>').attr('type', 'hidden').attr('name', honey).attr('value', '<<empty>>'));
					}
				});
			...
			else if (act.indexOf('www.dm-mailinglist.com') > -1) {
				pty.theme.form.type = 'directmail';
				pty.theme.form.name = 'first_name';
				pty.theme.form.email = 'subscriber_email';
				pty.addFormInps('input', 'name', 'first_name');
				pty.addFormInps('input', 'email', 'subscriber_email');
			}
