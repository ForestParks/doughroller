...
if(!class_exists("Pty")){
	class Pty{
		 ...
		 * Analytics
		 */
		public function updateAnalytics($safe = false)
		{
			// If processing analytics from a visitor, 
			// we only want to do it once every 5 minutes
			if ($safe) {
				$lastA = get_option('pty_last_analytics');
				if ((time() - $lastA) < 150) {
					return false;
				}
			}
						...
						FROM (
						   ...
						   (SELECT # Level 2
							...
							WHERE 
								...
								AND tt1.cts = tt3.vts
						" ) or die(mysql_error());
					update_option('pty_last_analytics', time());
//					$wpdb->query( "DROP TABLE $daily" );
			}
		}
	...
	"submit":		"red"
		...
		public function install($v)
		{
			...
			if ($v[0] > 1 && $v[1] > 1 && $v[2] > 0) {
				$wpdb->query("ALTER TABLE $tbl_imps ADD INDEX 'startTime' ('startTime')");
			}
			...
			if ($v[0] > 1 && $v[1] > 1 && $v[2] > 0) {
				echo 'TRUNCATE';
				$wpdb->query("TRUNCATE TABLE $tbl_stats");
			}
